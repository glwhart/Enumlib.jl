...
```bash
curl -sSL https://github.com/glwhart/Enumlib.jl/releases/latest/download/enumlib-jl-0.3.7-linux-x86_64.tar.gz | tar xz
export PATH="$PWD/enumlib-jl-0.3.7-linux-x86_64/bin:$PATH"
enum.x --version      # enum.x (Enumlib.jl) 0.3.7
```
...
```bash
xattr -dr com.apple.quarantine enumlib-jl-0.3.7-macos-aarch64
```
