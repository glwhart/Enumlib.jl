...
[^2]: `enumerate_structures(...)` also runs this check internally and refuses to start when the predicted memory exceeds `memory_budget`. See [Dispatch and the resource check](../explanation/dispatch-and-cost-gate.md).
