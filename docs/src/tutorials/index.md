...
4. **[Multilattice with per-sublattice concentration](04-multilattice-per-sublattice.md)** — Regime C: heterogeneous sublattices with distinct species, and the v0.3+ `Concentration(sites, per_sublattice)` constructor that states composition per dset position. ~10 minutes.
