...
**Why wurtzite diverges from Fortran (3 vs 4 at n=1):** The 12-op label-aware group includes the 6₃ screw, which maps cation d₁ ↔ d₂ (and anion d₃ ↔ d₄). Spacey identifies this correctly — `op 5` of the 12 has `R = -60°` rotation and `τ = (1/3, 2/3, 1/2)`, and one can verify by hand that `R · d₁ + τ ≡ d₂ (mod L)`. Under this op the binary cation labelings `(0, 1)` and `(1, 0)` are in the same orbit, giving 3 inequivalent configs at n=1. Fortran's enumeration uses a *smaller* symmetry group (a symmorphic subgroup of P6₃mc — likely P3m1 or P6mm — without the 6₃ screw), so it sees the two labelings as inequivalent and gets 4.

**Resolution (2026-05-23 — user direction):** Fortran is wrong, Julia is correct. The case that makes this clear is exactly the per-inactive-site control we're adding in Regime C: if a user can declare distinct inactive sublattices through `Sites.allowed_labels`, the symmetry group must respect that distinction. Wurtzite's 6₃ screw maps cation₁ → cation₂ and anion₁ → anion₂ — it preserves the label-class structure, so it's a legitimate symmetry of the labeled structure and must be kept. Fortran dropping it amounts to over-distinguishing positions that are actually equivalent under a real space-group operation. Julia's 12-op group is the right answer for any user who wants the full crystallographic symmetry of their labeled structure; Fortran's narrower group was an artifact of Fortran enumlib not supporting per-position `allowed_labels` in the first place (every site had the same allowed set, so the per-position distinction the 6₃ screw exploits couldn't be expressed).

This is a permanent semantic divergence between the two implementations. The wurtzite Fortran-corpus rows are no longer load-bearing for correctness; the testset asserts the parent-level filter shape (24 → 12) instead of matching the Fortran enumeration count. The migration guide (Phase 13f) should call this out for users moving from Fortran enumlib — their previous wurtzite-class counts will not match Enumlib.jl, and Enumlib.jl's are the mathematically correct ones for the full space group.

### 11.5 Chunk 6.5a — `:multinomial_restricted` (2026-05-19)

Shipped as v0.2.1 follow-up: `src/algorithms/multinomial_restricted.jl` exports `getUniqueColorings_multinomial_restricted(perm_group, multiplicities, site_mask)`. Wired into `src/enumerate.jl` via `_enumerate_multinomial_restricted` and accepted as `algorithm = :multinomial_restricted`. Public API surface is unchanged otherwise.

**Implementation tradeoff (initial landing):** linear iteration over the full multinomial bitmap with a lazy mask check per slot. Simple, correct, and identical in structure to chunk-6's `:multinomial` plus one extra `if !_coloring_satisfies_mask(...) continue` line. The downside is that the algorithm scales by the *full* multinomial coefficient, not by the valid-coloring subspace — for sparse masks (the Heusler / wurtzite / perovskite family where most positions are inactive) the bitmap-allocation and per-slot mask check dominate. At n=5 in half-Heusler Y=Z={2}, the worst-case multinomial is 756 756 with most slots invalid, and at n=7 it's 3.99e8.

`:auto` dispatch consequently **does not pick** `:multinomial_restricted` for Regime C today — it stays with `:recursive_stabilizer` (whose tree scales by the valid subspace). Users opt into `:multinomial_restricted` explicitly when they have reason to believe the mask is dense (e.g. zinc-blende, where every position is active with disjoint per-sublattice labels — small enough that the bitmap dominates the tree's per-node overhead).

A tree-walk variant that prunes at branching time (HF 2012 §A.1's "skip disallowed states" framing) — scales by valid subspace, allocates no bitmap — is queued as a v0.3 perf-polish item if production usage demands it.

**Cross-validation tests** in `test/test_enumerate.jl` (chunk-6.5a testset) compare `:multinomial_restricted` against `:recursive_stabilizer` count-by-count across zinc-blende (n=1..3), half-Heusler Y=Z={2} (n=1..3), and perovskite (n=1..2). Equality holds in all cases. (Set-equality on labelings doesn't hold — the two algorithms pick different canonical orbit reps. Count equality is the load-bearing correctness assertion.) The testset also asserts the Regime A / Regime B rejection and the "requires concentration" gate.
